...
import { BarStackedChartData, BarStackedChartParams } from "@/types/expenses";
...
export const getBarStackedChartData = async (
  ...
  try {
    ...
    // Realizar la petición HTTP usando apiClient en lugar de axios
    const response = await apiClient.get<BarStackedChartData>(
      `/api/expenses/stacked_bar_chart_data/?${queryParams.toString()}`
    );

    return response.data;
  } catch (error) {
    console.error(
      "Error al obtener datos de gráfica de barras apiladas:",
      error
    );
