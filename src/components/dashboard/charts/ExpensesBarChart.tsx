...
const ExpensesBarChart: React.FC<ExpensesBarChartProps> = ({
  ...
  const enhancedWeeklyData: WeeklyExpenseData[] = React.useMemo(() => {
    if (viewMode === "month" && selectedMonth !== "year") {
      const baseData =
        weeklyDataByMonth[selectedMonth as keyof typeof weeklyDataByMonth] ||
      ...
      return baseData.map((week) => ({
        ...
        transport: Math.round(week.value * 0.2), // 20% transport
  ...
  const handleBarClick = (data: { date: string }, index: number) => {
        ...
        <div className="flex-1 min-h-[200px] flex items-center justify-center">
          <ResponsiveContainer width="100%" height={isMobile ? "90%" : "95%"}>
            <BarChart
              ...
              margin={
                isMobile
                  ? { top: 5, right: 5, left: -25, bottom: 15 }
                  : { top: 20, right: 20, left: 0, bottom: 15 }
              }
